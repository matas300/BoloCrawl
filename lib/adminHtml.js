...
function renderAdmin(bookings) {
  ...
  const rows = bookings.map(b => `
    <tr>
      ...
      <td>${esc(b.lang)}</td>
      <td>${b.id ? `<form method="post" action="/admin" onsubmit="return confirm('Eliminare definitivamente questa prenotazione?')">
        <input type="hidden" name="deleteId" value="${esc(b.id)}" />
        <button class="del" type="submit" title="Elimina prenotazione">✕</button>
      </form>` : ''}</td>
    </tr>`).join('');
...
form{margin:0}
.del{background:transparent;border:1px solid #4a2a32;color:#f87171;border-radius:6px;padding:3px 9px;cursor:pointer;font-size:13px;line-height:1.4}
.del:hover{background:#f87171;color:#1c1c26;border-color:#f87171}
</style></head>
<body><div class="wrap">
<div class="bar"><h1>🍻 Admin — Bolo Crawl</h1>
<a class="btn" href="/api/export.csv">Export CSV</a></div>
<div class="stats">
  ${stat('Prenotazioni', total)}
  ${stat('Persone totali', totalPeople)}
  ${stat('Incasso previsto', '€' + revenue)}
  ${stat('Media pax/prenotazione', total ? (totalPeople / total).toFixed(1) : '0')}
</div>
<div class="grid2">
  <div><h2>Lingua dei visitatori</h2><table><tr><th>Lingua</th><th>#</th></tr>${kvRows(byLang)}</table></div>
  <div><h2>Prenotazioni per giorno della settimana</h2><table><tr><th>Giorno</th><th>Persone</th></tr>${kvRows(byWeekday)}</table></div>
</div>
<h2>Top 10 date prenotate (per numero persone)</h2>
...
<table>
<thead><tr><th>Ricevuta</th><th>Data pub crawl</th><th>Pax</th><th>Nome</th><th>Email</th><th>Telefono</th><th>Lang</th><th></th></tr></thead>
<tbody>${rows || '<tr><td colspan=8>Nessuna prenotazione ancora.</td></tr>'}</tbody>
</table>
</div></body></html>`;
}

// Excel e Google Sheets interpretano come formula ogni cella che inizia con
// = + - @ (o tab/CR): un nome tipo "=HYPERLINK(...)" verrebbe eseguito
// all'apertura del CSV. Un apostrofo iniziale la forza a testo.
...
function toCsv(bookings) {
  const headers = ['createdAt', 'date', 'people', 'name', 'email', 'phone', 'lang'];
  return [
    headers.join(','),
    ...bookings.map(b => headers.map(h => csvCell(b[h])).join(','))
  ].join('\n');
}

module.exports = { renderAdmin, toCsv, csvCell, esc };
