...
const { checkBasicAuth, REALM } = require('./lib/adminAuth');
...
// Ultimo in catena: risponde a tutto ciò che nessuna route ha gestito.
app.use((req, res) => {
  res.status(404);
  res.set('Content-Type', 'text/html; charset=utf-8');
  res.send(render404(translations[detectLang(req)]));
});
...
app.listen(PORT, () => {
  ...
  if (!process.env.ADMIN_PASS) {
    console.warn('   ⚠️  ADMIN_PASS non impostata: /admin e /api/export.csv rispondono 401.');
  }
});
